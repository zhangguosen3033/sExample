...
- (UILabel *)marqueeLbl{
    if (!_marqueeLbl) {
        ...
        if (self.txtColor ==nil) { //如果颜色传空 表示处理富文本
            
            NSAttributedString * attrStr = [[NSAttributedString alloc] initWithData:[self.msg dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType } documentAttributes:nil error:nil];
            [_marqueeLbl setAttributedText:attrStr];
...
- (void)addLeftAndRightGradient{
    CGFloat w = self.frame.size.width;
    CGFloat h = self.frame.size.height;
    SXColorGradientView *leftFade = [SXColorGradientView createWithColor:self.bgColor frame:CGRectMake(0, 0, h+1, h) direction:SXGradientToRight];
    self.leftFade = leftFade;
    
    SXColorGradientView *rightFade = [SXColorGradientView createWithColor:self.bgColor frame:CGRectMake(w - h, 0, h+10, h) direction:SXGradientToLeft];
